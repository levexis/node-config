...
var Config = function() {
  ...
  DISABLE_RUNTIME = t._initParam('NODE_CONFIG_ENABLE_RUNTIME') ? false : true;
  NODE_ENV = t._initParam('NODE_ENV', 'development');
  CONFIG_DIR = t._initParam('NODE_CONFIG_DIR', process.cwd() + '/config');
  RUNTIME_JSON_FILENAME =  DISABLE_RUNTIME ?  '' : t._initParam('NODE_CONFIG_RUNTIME_JSON', CONFIG_DIR + '/runtime.json');
  DISABLE_FILE_WATCH =  DISABLE_RUNTIME || t._initParam('NODE_CONFIG_DISABLE_FILE_WATCH');
  PERSIST_ON_CHANGE = DISABLE_RUNTIME ? false : t._initParam('NODE_CONFIG_PERSIST_ON_CHANGE');
  APP_INSTANCE = t._initParam('NODE_APP_INSTANCE');
  HOST = t._initParam('HOST');
  HOSTNAME = t._initParam('HOSTNAME');
  t._loadFileConfigs();
  if (PERSIST_ON_CHANGE) t._persistConfigsOnChange();
};
...
Config.prototype.watchForConfigFileChanges = function(interval) {
   ...
   // this sends nodemon wild!
   if (DISABLE_RUNTIME || 
        interval === 0 || 
        DISABLE_FILE_WATCH === "Y") return;
 ...
 */
Config.prototype._loadFileConfigs = function( refresh ) {

  // Initialize
  var t = this;

  // Run only once
  if (originalConfig && !refresh) {
    return t;
  }
...
Config.prototype._parseFile = function(fullFilename) {
  ...
  var checkRuntimeJson = function() {
    if ( !DISABLE_RUNTIME && fullFilename == RUNTIME_JSON_FILENAME) {
      try {
 ...
 *
 * <p>
 * This method resets the original config.
 * </p>
 * @method setOriginalConfig
 * @param {string} NODE_ENV (optional )
 * @return {object} The original config object is returned
 */
Config.prototype.reset = function( env ) {
    NODE_ENV = env || process.NODE_ENV || NODE_ENV;
//    console.log('resetting to', NODE_ENV);
    return this._loadFileConfigs( true );
}
